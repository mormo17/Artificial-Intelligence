...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        if not self.getLegalActions(state):
          ...
          for possible_action in self.getLegalActions(state):
            result_value = max(result_value, self.getQValue(state, possible_action))
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        to_set = (1-self.alpha) * self.getQValue(state, action) + self.alpha * (reward + self.discount * self.getValue(nextState))
        self.q_values[state][action] = to_set
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for (feature, value) in self.featExtractor.getFeatures(state, action).items():
          result_value += self.weights[feature] * value
