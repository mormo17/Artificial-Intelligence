...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance, pacmanPosition, ghostPosition, jailPosition):
        ...
        """
        noisyDistanceIsNone = (noisyDistance is None)
        ghostInJail = (ghostPosition == jailPosition)

        if noisyDistanceIsNone:
            if ghostInJail:
                return 1.0
            else:
                return 0.0
        ...
        observationProb = busters.getObservationProbability(noisyDistance, manhattanDist)
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for currPos in self.allPositions:
            observationProb = self.getObservationProb(observation, pacmanPos, currPos, jailPos)
            self.beliefs[currPos] *= observationProb
    ...
    def elapseTime(self, gameState):
        ...
        """
        newBeliefsToSet = DiscreteDistribution()
        
        for prevPos in self.allPositions:
            currPositionDist = self.getPositionDistribution(gameState, prevPos)
            prevProb = self.beliefs[prevPos]
            for currPos in currPositionDist.keys():
                currProb = currPositionDist[currPos]
                newBeliefsToSet[currPos] += prevProb * currProb 
        
        self.beliefs = newBeliefsToSet
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        self.particles = []
        for indx in range(0, self.numParticles):
            self.particles.append(self.legalPositions[indx % len(self.legalPositions)])
    ...
    def elapseTime(self, gameState):
        ...
        for particle in self.particles:
           ...
           possiblePosition = posDist.sample()
           particles.append(possiblePosition)
...
class JointParticleFilter(ParticleFilter):
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for indx in range(self.numGhosts):
                agent = self.ghostAgents[indx]
                posDist = self.getPositionDistribution(gameState, oldParticle, indx, agent)
                sample = posDist.sample()
                newParticle[indx] = sample
